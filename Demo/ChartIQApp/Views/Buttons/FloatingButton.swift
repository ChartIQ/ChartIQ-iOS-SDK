...
extension FloatingButtonPosition {
  ...
  internal func cgRectValue(parentView: UIView) -> CGRect {
    ...
    let bottomDistance = Const.FloatingButton.bottomMargin + Const.Chart.fullViewFloatingButtonSize.width
    let rightDistance = Const.FloatingButton.rightMargin + Const.Chart.fullViewFloatingButtonSize.width
    ...
    case .rightTop:
      return CGRect(origin: CGPoint(x: rightWidthXPosition, y: topDistance),
                    size: Const.Chart.fullViewFloatingButtonSize)
    case .rightBottom:
