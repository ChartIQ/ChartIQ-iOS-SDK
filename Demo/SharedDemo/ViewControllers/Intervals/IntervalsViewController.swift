...
class IntervalsViewController: BaseViewController {
  ...
  private func setupIntervals() {
    intervals = [
      .first: [
        // IntervalModel(periodicity: 1, interval: 1, timeUnit: .tick),
        IntervalModel(periodicity: 1, interval: 1, timeUnit: .day),
        IntervalModel(periodicity: 1, interval: 1, timeUnit: .week),
        IntervalModel(periodicity: 1, interval: 1, timeUnit: .month)
      ],
      .second: [
        IntervalModel(periodicity: 1, interval: 1, timeUnit: .minute),
        IntervalModel(periodicity: 1, interval: 5, timeUnit: .minute),
        IntervalModel(periodicity: 1, interval: 10, timeUnit: .minute),
        IntervalModel(periodicity: 3, interval: 5, timeUnit: .minute),
        IntervalModel(periodicity: 1, interval: 30, timeUnit: .minute),
        IntervalModel(periodicity: 2, interval: 30, timeUnit: .hour),
        IntervalModel(periodicity: 8, interval: 30, timeUnit: .hour)
      ],
      .third: [
        IntervalModel(periodicity: 1, interval: 30, timeUnit: .second)
      ]
    ]
    tableView.reloadData()
  ...
  private func getIntervalModelFromPickerView() -> IntervalModel? {
    if let timeSelectedRow = pickerView?.selectedRow(inComponent: 0),
      ...
      return IntervalModel(periodicity: 1, interval: interval, timeUnit: timeUnit)
    }
    return nil
  }
